...
actual class SharedImage(private val image: UIImage?) {
    ...
    actual fun toByteArray(): ByteArray? {
        return if (image != null) {
            val imageData = UIImageJPEGRepresentation(
                image = image,
                compressionQuality = COMPRESSION_QUALITY,
            ) ?: throw IllegalArgumentException("image data is null")

            val bytes = imageData.bytes ?: throw IllegalArgumentException("image bytes are null")
    ...
    actual fun toImageBitmap(): ImageBitmap? {
        ...
        return if (byteArray != null) {
            Image.makeFromEncoded(bytes = byteArray).toComposeImageBitmap()
        } else {
            null
        }
    }
